...
- **brush-up 第4ラウンド(2026-06-26・`/reviewing-multi-angle medium` 8 件)**: すべて採用
  - #1(🔴 blocker): TREE タブで search が `.file` 選択を除外したとき、行版 `reconcile(previous:in:)` が `visibleRows.first`(通常ルート dir 行)を返し `selection = .dir(...)` に倒れ preview が消える非対称を解消。`searchText.didSet` と `rescan` の TREE 経路で **`previous` が `.file` の場合は leaf 版 `reconcile(in: visibleLeaves)` に分岐**して file → file の遷移を保つ。`.dir` 選択時のみ行版を使う。回帰テスト `reconcileRowMayFallToDir` を追加(行版が file→dir に倒れる Core 動作を明示記録)
  - #2(🟡 YAGNI): `TreeRow.dir(id: String, depth: Int)` の `depth` フィールドは UI 側で未参照(`DisclosureGroup` がネイティブにインデント)で書き込みのみの dead field だったため、**`TreeRow.dir(id: String)`** に縮約。`walk` の引数も削減、`SelectionLogic.selection(of:)` も簡素化、テスト fixture からも除去
  - #3(コメント因果ループ): `selectedFile` setter の WHY コメントが「setter が `.dir` 選択を保護する」と誤読される構造だったのを修正。実際の `.dir` 保護は `rescan` / `recomputeTreeExpansion` 等の **`selection == nil` 判定経路**で行う(round-1/2 で実装済み)。preview が消えないのは WKWebView が前回 loadFileURL を保持しているため、と明示
  - #4(コメント事実不一致): `moveSelection` RECENT 側コメントの「fullOrder 補正で隠れた選択救済」が、`visible==fullOrder==recentFiles` で実質作用しない名目記述だったため正直化
  - #5(default の片手落ち): `rescan` の stale 判定 switch を `case .file(...) where !f.isExternal / case .file / case .dir / case .none` の **全 case 明示分岐**に変更。将来 `SidebarSelection` に新 case を追加したとき `default` ですり抜けるリスクを除去
  - #6(マジックナンバー): `installKeyMonitor` の `keyCode` 判定で 125/126/36/76 を **`Carbon.HIToolbox` の `kVK_DownArrow` / `kVK_UpArrow` / `kVK_Return` / `kVK_ANSI_KeypadEnter`** に置換。将来のキー追加時の typo 防止
  - #7(テスト分割): `nextRowMixedClamp` の 5 観点を `nextRowAdjacentMixed`(隣接遷移 3)+ `nextRowClamp`(端クランプ 2)に分離。Swift Testing は `#expect` 失敗で継続実行されるが、`@Test` 名で観点が分かる方が診断が早い
  - #8(診断メッセージ): `TreeBuilderTests.visibleRows*` の `#expect(Bool(false))` 計 8 箇所に `"rows[N]=\(rows[N])"` の説明を付与。回帰時にどの行が想定外か即座に分かるよう改善
  - 計 99 green / `make check` pass / `swift build` clean

### sidebar-click-fix + autoscroll(#36 + #37 / 2026-06-28)

#33(sidebar-keynav)マージ後の実機検証で **クリックで selection が変わらない**ことが判明し、`docs/04 §5 M7 #13` の ✅ 2026-06-26 が誤判定だったと発覚。同時に「選択が範囲外に動いたとき自動スクロールしない」(#37)も改善対象に。両者は同じ `SidebarView` の List 周辺を触るため 1 PR にまとめて対応する。

- **#36 クリック選択 fix**:
  - **真因**: PR #33 で入れた `@FocusState var listFocused` + `.focused() / .onTapGesture { listFocused = true }` は SwiftUI 論理フォーカスを立てるだけで AppKit 層の `makeFirstResponder` を呼ばない。プレビュー WKWebView が key first responder を握ったままだと `NSTableView.firstClickValid=false` 挙動で 1 回目のクリックがフォーカス遷移だけで消費され selection が動かない macOS 標準挙動を、設計が救えていなかった
  - **修正**: `@FocusState var listFocused` を撤去(誰からも読まれていない宙吊り state だった)。List 自体の `.onTapGesture` は削除し、**各行(`FileRowView` / `TreeRowsView` の file/dir label)に `.onTapGesture` を当て**、`app.selection = ...` を SwiftUI 内で直接書き込み、同時に `NSApp.keyWindow?.makeFirstResponder(nil)` で WKWebView から first responder を AppKit 直叩きで外す
  - **副次効果**: クリック後に `keyEventShouldYieldToFocus()` が false を返すようになり、↑↓ / j/k がサイドバー操作として扱われる(従来は WebView に流れていた可能性が高い)
- **#37 自動スクロール feat**:
  - List 全体を `ScrollViewReader { proxy in ... }` で wrap。各行に `.id(SidebarSelection)` を付与(scrollTo の解決対象)
  - `.onChange(of: app.selection)` で `proxy.scrollTo(new, anchor: nil)` を `withAnimation(.easeOut(duration: 0.15))` 付きで呼ぶ。`anchor: nil` は SwiftUI の「可視範囲内なら no-op、外なら最寄り端寄せ」挙動で、Finder 流儀に一致
  - `.onChange(of: app.selectedTab)` でタブ切替時にも追従(同じ selection が新タブで隠れていれば可視化)
- **docs**:
  - `docs/02-specification.md` §2: 「選択が表示範囲外に動いたら自動スクロール(Finder 流儀)」を 1 行追加
  - `docs/04-verification.md` §5 M7 #13 を `✅ 2026-06-28(#36 で本物の修正 — #32 時点は誤判定だった)` に書き換え、#16 自動スクロール行を新規追加 `✅ 2026-06-28(#37 実機確認)`
- **学び**: PR #33 の検証で「起動直後の selection 未設定 → 1 クリック → selection 変化」最小手順を踏まず、既存 selection 残存状態で確認したため誤判定。**UI 系の ✅ は「初期化済み状態から 1 操作」で確認する**を以後の検証ルールに(`reasoning trail` として残す)。`@FocusState` ≠ AppKit makeFirstResponder という SwiftUI の壁にも今回ぶつかったが、AppKit 直叩きで吸収する逃げ道があると分かったのが収穫
- **brush-up 第1ラウンド(2026-06-28・`/reviewing-multi-angle medium` 4 件 / `merge_ready` 判定)**: 採用 4(blocker 0)
  - #1 / #3(🟡 / 🟢): `makeFirstResponder(nil)` が SidebarView と TreeRowsView の計 3 箇所に散在 + WKWebView 奪取コメントが 3 箇所で部分的に重複していたのを、**file スコープの `fileprivate func yieldFirstResponderFromWebView()`** に集約。canonical な WHY は同関数の docstring 1 箇所に、呼び出し側コメントは「奪取の WHY は同関数の docstring 参照」で pointer 化。SidebarView (struct) の private method 版を撤去し、TreeRowsView からも直接呼べる構造に統一(同期漏れリスク排除)
  - #2(🟡): `yieldFirstResponderFromWebView()` に **`os.Logger`(subsystem=`com.hayashi.htmlviewer`, category=`firstResponder`)** で観測点を追加。奪取前の `firstResponder` クラス名と `makeFirstResponder(nil)` の戻り値(accepted)を 1 行 `Logger.debug` で残す。`Console.app` から filter で観察でき、再発時の「WKWebView ⇄ サイドバー継ぎ目」切り分けコストを下げる
  - #4(🟢): `withAnimation(.easeOut(duration: 0.15))` が selection 変更時とタブ切替時の 2 箇所マジック数だったのを、**`private let scrollAnimationDuration: Double = 0.15`**(file スコープ)で集約。片方だけ修正で非対称化するリスクを排除
  - 計 99 green / `make check` pass / `swift build` clean
